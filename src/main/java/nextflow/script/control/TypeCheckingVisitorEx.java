...
public class TypeCheckingVisitorEx extends ScriptVisitorSupport {
    ...
    public void visitFeatureFlag(FeatureFlagNode node) {
        ...
        if( !TypesEx.isAssignableFrom(expectedType, actualType) )
            addError("Type mismatch for feature flag '" + node.name + "' -- expected a " + TypesEx.getName(expectedType) + " but received a " + TypesEx.getName(actualType), node);
    }
    ...
    public void visitMethodCallExpression(MethodCallExpression node) {
        ...
        if( defNode instanceof ProcessNodeV1 || defNode instanceof WorkflowNode )
